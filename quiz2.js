...
const main = () => {
  ...
  arrayString.forEach((x) => {
    console.log(x);
    x = replaceFirstWordNumber(x, dictionary);
    x = replaceLastWordNumber(x, dictionary);
    let numberInRow = x.replace(/\D+/g, "");
    numberInRow = Number(numberInRow);
    let firstDigit = 0;
    let secondDigit = 0;
    let numberString = String(numberInRow);
    firstDigit = Number(numberString[0]);
    secondDigit = Number(numberString[numberString.length - 1]);
    console.log(firstDigit + "  " + secondDigit);
    sumCalibration += firstDigit * 10 + secondDigit;
  });
  console.log(sumCalibration);
};

function replaceFirstWordNumber(str, dictionary) {
  ...
  if (replaceKey !== null) {
    return str.replace(replaceKey, dictionary.get(replaceKey));
  }
  return str;
}

function replaceLastWordNumber(str, dictionary) {
  let lastIndex = -1;
  let replaceKey = null;
  dictionary.forEach((value, key) => {
  ...
  if (replaceKey !== null) {
    return (
      ...
      dictionary.get(replaceKey) +
      str.substring(lastIndex + replaceKey.length)
    );
  }
  return str;
}

const data1 = `two1nine
  eightwothree
  abcone2threexyz
  x3fourtwone
  4nineeightseven2
