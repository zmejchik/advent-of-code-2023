const main = () => {
  ...
  let arrayGames = data1.split(/\r\n|\r|\n/g);
  let sumGame = 0;
  arrayGames.forEach((str) => {
    let arrayGame = str.split(":");
    let identificator = getIdentificator(arrayGame[0]);
...
function chekTrueGame(colorsCount, red, green, blue) {
  if (
    colorsCount.red <= red &&
    colorsCount.green <= green &&
    colorsCount.blue <= blue
  ) {
    return true;
  }
  return false;
...
function countColors(str) {
  ...
  const items = str.match(/\d+ [a-z]+/gi);
  items.forEach((item) => {
    const [quantity, color] = item.split(" ");
    if (color.includes("red")) {
      colors.red += parseInt(quantity);
    } else if (color.includes("blue")) {
      colors.blue += parseInt(quantity);
    } else if (color.includes("green")) {
      colors.green += parseInt(quantity);
    }
  });
  console.log(colors);
  return colors;
